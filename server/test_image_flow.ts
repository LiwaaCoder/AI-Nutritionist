
import dotenv from 'dotenv';
import { analyzeInput } from './geminiAgent';
import { GoogleGenAI } from '@google/genai';

// Load environment variables
dotenv.config({ path: '../.env.local' });

async function testImageFlow() {
    console.log("Starting Image Flow Test...");

    const userId = "test-user-image-flow";

    // 1x1 Pixel Red Dot Base64 JS (PNG) - wait, let's use a JPEG to be safe, but code should now handle PNG too.
    // Small white JPEG
    const dummyBase64 = "data:image/jpeg;base64,";

    console.log("Calling analyzeInput with dummy base64 image...");

    try {
        const response = await analyzeInput(
            userId,
            "What is in this image?",
            dummyBase64
        );

        console.log("\n--- AGENT RESPONSE ---");
        console.log(JSON.stringify(response, null, 2));
        console.log("----------------------\n");

        if (response.message) {
            console.log("SUCCESS: Agent responded.");
        } else {
            console.error("FAILURE: Agent returned empty message.");
        }

    } catch (error) {
        console.error("CRITICAL ERROR during test:", error);
    }
}

testImageFlow();
